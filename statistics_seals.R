...
finaldata_adults <- read_excel("finaldata_adults.xlsx") #version where NND's were measures after removing indiv.<0.2m2 from Eemsdollard
inaldata_adults <- read_excel("finaldata_adults.xlsx")
...
#determine adults size limit eemsdollard
...
## Change part of noorderhaaks to HG ##
noorderhaaks = subset(dat, dat$`haul-out site`=="noorderhaaks")
...
## new data after changing the species
dat2 = rbind(pv, hg, noordh.mix, noordh.hg)
...
# switch hg/pv and orig.nnd/rndm.nnd manually
sd(pv3$`number of seals within 5m distance`)
std.error(pv3$`NND rndm KDE (m)`)
quantile(pv3$`NND rndm KDE (m)`, probs = seq(.9, by = .1))
...
# Mann Whithney U test ( = non-paired wilcox)
# data vs random a / b  -  all data
...
dat3 = subset(dat2, dat2$species != "mix") #remove mix 
...
# 3. density plots, boxplots GGPLOT
...
#dates
### deze klopt niet want zit ook RANDOM nog in!!
ggplot(data = glmdata.pv,
